            ...
            async with websockets.connect(WEBSOCKET_URL) as websocket:
                ...
                async for message in websocket:
                    ...
                    try:
                        ...
                        message_text = (
                            f"🎮 <b>Kết quả phiên hiện tại:</b> <code>{escape_html(result)}</code>\n"
                            f"🔢 <b>Phiên:</b> {session} → {next_session}\n"
                            f"{dice_display} <b>Tổng:</b> {total}\n"
                            f"🤖 <b>Dự đoán phiên tiếp theo:</b> <code>{escape_html(predicted_result)}</code>\n"
                            f"📌 <b>Lý do:</b> {escape_html(reason)}\n"
                            f"🧩 <b>Mô hình sử dụng:</b> {escape_html(best_pattern_type)}\n"
                            f"👑🟢<b>Thanh Tùng VIP</b>👑🟢\n"
